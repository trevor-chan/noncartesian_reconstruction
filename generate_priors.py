...
'''
Usage
python generate_priors.py --kspacedir=fastMRIprocessing/data_small
...
def complex_to_magphase(array_complex):
...
def load_raw_image(fname): #Copied from dataset.py dataset class
    ...
    image_2ch = complex_to_magphase(image_complex)
    prior_2ch = complex_to_magphase(prior_complex)
    ...
    # PIL.Image.fromarray((prior_2ch[0,:,:]*127.5+127.5).astype(np.uint8),'L').save('out/prior_real_{}.png'.format(name))
    # PIL.Image.fromarray((prior_2ch[1,:,:]*127.5+127.5).astype(np.uint8),'L').save('out/prior_imag_{}.png'.format(name))
    # PIL.Image.fromarray((image_2ch[0,:,:]*127.5+127.5).astype(np.uint8),'L').save('out/image_real_{}.png'.format(name))
    # PIL.Image.fromarray((image_2ch[1,:,:]*127.5+127.5).astype(np.uint8),'L').save('out/image_imag_{}.png'.format(name))
...
def main(kspacedir, interleave_min, interleave_max, undersampling, alpha_min, alpha_max):
    ...
    for kspacefile in tqdm.tqdm(kspacelist):
        ...
        image_magnitude = ((image_magnitude + 1) * 127.5).clip(0, 255).astype(np.uint8).transpose(1, 2, 0)
        prior_magnitude = ((prior_magnitude + 1) * 127.5).clip(0, 255).astype(np.uint8).transpose(1, 2, 0)
        image_phase = image_2ch[1][np.newaxis, :, :]
        prior_phase = prior_2ch[1][np.newaxis, :, :]
        image_phase = ((image_phase + 1) * 127.5).clip(0, 255).astype(np.uint8).transpose(1, 2, 0)
        prior_phase = ((prior_phase + 1) * 127.5).clip(0, 255).astype(np.uint8).transpose(1, 2, 0)
        ...
        image_savename = os.path.splitext(os.path.split(kspacefile)[1])[0]+'_gt.png'
