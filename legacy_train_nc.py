...
python train_nc.py --outdir=training-runs --data=fastMRIprocessing/data_small --batch=128
"""
...
def main(**kwargs):
    ...
    c = dnnlib.EasyDict()
    c.dataset_kwargs = dnnlib.EasyDict(class_name='legacy_noncart_training.dataset.NonCartesianDataset', path=opts.data, use_labels=opts.cond, xflip=opts.xflip, cache=opts.cache, undersampling=opts.undersampling, fetch_raw=opts.fetch_raw)
    ...
    if 1: #Currently only configured for edm preconditioning and customized conditional EDM loss
        ...
        c.loss_kwargs.class_name = 'legacy_noncart_training.loss.ConditionalEDMLoss'
